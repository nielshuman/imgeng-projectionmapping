...
DEBUG = False  # <--- Add this line. Set to False to disable debug windows.
...
def opencv_thread():
        ...
        while True:
                ...
                if state == "white":
                        ...
                        cv2.putText(vis, "Laser tracking mode", (10, 30),
                                                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 100, 255), 2)

                        if DEBUG:
                                cv2.imshow("Debug - threshold laser", thresh)

                        # ── NEW: send laser point to frontend ─────────────────────────────
                        if point is not None:
                                with lock:
                                        H = shared["H"]

                                if H is not None:
                                        cx, cy = point  # camera coords
...
async def websocket_endpoint(ws: WebSocket):
        ...
        try:
                while True:
                        ...
                        # ── Client-triggered recalibration ────────────────────────
